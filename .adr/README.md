...
data-migration runner, the tenancy model and the custom Express entrypoint. All
four subsystems were removed when the site was stripped down to a prerendered
static build, so the records went with them. `ts-factory-stack` still carries
them if you need the reasoning.
