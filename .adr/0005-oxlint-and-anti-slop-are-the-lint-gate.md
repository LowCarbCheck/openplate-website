...
why the starter's boundary helpers exist in the shapes they do: a schema per
call, decoding into a named type, rather than a cast. This site has few
boundaries left, but the ones the docs sync adds in M193 spec 02 are held to the
same rule. Removing this ADR would not just relax a linter; it would strand that
architecture without its justification.
