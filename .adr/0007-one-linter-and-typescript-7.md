...
- The import guardrails that forced typescript-eslint move into
  `.oxlintrc.json` `overrides`, which is what unblocked the compiler upgrade.
  (This repository, cloned from the starter and stripped to a static site, has
  no package boundary left to guard, so it carries no `overrides` entry at all.
  The mechanism is what the record is about.)
...
honors them, so the legitimate boundary exceptions in the starter were left
alone rather than churned to `oxlint-disable`.
...
each banned import pattern in an in-scope directory, plus negative controls in
out-of-scope trees. All fired and stayed silent as expected.
